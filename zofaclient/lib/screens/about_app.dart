...
class AboutAppScreen extends StatelessWidget {
  ...
  Future<void> _launchURL(String url) async {
    ...
    try {
      if (await canLaunchUrl(uri)) {  // Check if the URL can be launched
        await launchUrl(uri, mode: LaunchMode.externalApplication);  // Try to open in the app
      } else {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('אודות האפליקציה')),
      body: Directionality(
        textDirection: TextDirection.rtl,  // Force RTL for all children
        child: Padding(
          padding: const EdgeInsets.all(16.0),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,  // Align content to the right
            children: [
              // App name
              const Text(
              ...
              // LinkedIn link
              Row(
                mainAxisAlignment: MainAxisAlignment.start,  // Align icons and text to the right
                children: [
                  _buildIconButton(
                    iconUrl:
